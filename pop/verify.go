...
// jwk/x5c (acceptDPoP), x5c[0] P-256 leaf, jwk↔x5c key equality, signature
// under that single key, the certificate's validity period at now, htm,
// normalized htu, ath ⟺ presented token, iat window, then jti single-use.
// Replay is recorded LAST, so only proofs that pass every other check consume
// a cache slot.
//
